%% General Description
% This is the basic/template model to verify the DTC INL
% correction/calibration/cancellation/dithering techniques.

% Version 1.0, Peng CHEN, 2021.11.28

%% Initial Parameter Settings
clc;clear;close all;
Sim_Time = 2^18; % The number is in FREF cycles.
settle_sample = 1e5; % The unit is in CKV cycles.
...
step_DCO = 1;
I_path = 0;
yiir1 = 0; yiir2 = 0; yiir3 = 0; yiir4 = 0;lambda1 = 1/2^1;lambda2 = 1/2^1;lambda3=1/2^1;lambda4=1/2^1;
...
for step = 1:Sim_Time    
    ...
    while 1 %(tckv(step_DCO) < tR(step))        
        ...
        if (tckv(step_DCO) >  tR(step))      
            break            
        end        
        step_DCO = step_DCO + 1;        
    end    
    trise(step) = (tR(step) - tckv(step_DCO-1));    
    Tv_current(step) = tckv(step_DCO) - tckv(step_DCO-1);    
    if (N_TDC_avg >= step)
        Tv_est(step) = 1/N_TDC_avg*(sum(Tv_current(1:step))+(N_TDC_avg-step)*1/f0);
    else
        Tv_est(step) = mean(Tv_current(step-N_TDC_avg+1:step));
    end
    epsilon(step) = 1 - (floor(trise(step)/delta_TDC)-0.5)*delta_TDC/Tv_est(step);   
    phie(step) = Rr(step) - Rv(step_DCO) + epsilon(step);
