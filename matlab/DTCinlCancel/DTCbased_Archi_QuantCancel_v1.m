...
% cancellation.
% In the version, INL cancellation is available, but with FCW_F/BW limitation.

% Version 1.0, Peng CHEN, 2021.12.06
...
for step = 1:Sim_Time
    ...
    %DTC_delay(step) = DCW(step)*DTC_reso+DTC_INL(randi(length(DTC_DNL)));
    ...
    trise(step) = (-tR(step) + tckv(step_DCO)-200e-12);
    Tv_est = 1.0*Tv; % The TDC gain variation affects the BW, not INL.
    epsilon(step) = (floor(trise(step)/delta_TDC)-0.5)*delta_TDC/Tv_est - (CNT_cnt - FCW_I - OV_PHRF) - DCW_residue/DTC_gain_est ...
                    + INL_correction(DCW(step))/Tv_est;
