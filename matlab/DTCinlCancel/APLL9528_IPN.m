...
Lcp_dB_scp = 1*[-196 -213 -221 -222 -222.3] + 10*log10(NTCXO) ;
%XO
...
Sf_dsm = (1/(12*Fref)).*abs(((1-Z.^(-1)).^3)).^2;
...
IPN = 2*trapz(f(IPN_Range),10.^(So_tot_dB(IPN_Range)./10));
IPN_tot = IPN;

jitter = sqrt(IPN)/(2*pi*Fvco)
