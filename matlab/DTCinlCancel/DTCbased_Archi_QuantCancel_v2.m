...
% This version tries to fix the bug

% Version 2, Peng CHEN, 2021.12.07
...
f0 = 2.4e9;
T0 = 1/f0;
fcw = 50+1/2^11;%When FCW_F is too small, the INL calibration does not work. It is related to the PLL LF coefficients.
...
for step = 1:Sim_Time
    ...
    if ((prbs(step)==2)&& (PHRF<0.5))
        DCW(step) = floor((1-PHRF-0.5)*DTC_gain_est)+1;
        DCW_residue = (1-PHRF-0.5)*DTC_gain_est  - DCW(step);
    elseif ((prbs(step)==2) && (PHRF>= 0.5))
        DCW(step) = floor((1-PHRF+0.5)*DTC_gain_est)+1;
        DCW_residue = (1-PHRF+0.5)*DTC_gain_est - DCW(step);
    else
        DCW(step) = floor((1-PHRF)*DTC_gain_est)+1;
        DCW_residue = (1-PHRF)*DTC_gain_est - DCW(step);
    end
    %DCW(step) = floor((1-PHRF)*DTC_gain_est)+1;
...
%     end    
    ...
    epsilon(step) = (floor(trise(step)/delta_TDC)-0.5)*delta_TDC/Tv_est - (CNT_cnt - FCW_I - OV_PHRF) - 1*DCW_residue/DTC_gain_est + 1*INL_correction(DCW(step))/Tv_est;
    phie(step) =  epsilon(step);
    if ((step_DCO>settle_sample*0.5)) DTC_gain_est = DTC_gain;end%_est + 2*9e-1*phie(step)*(1-PHRF-0.5); end
