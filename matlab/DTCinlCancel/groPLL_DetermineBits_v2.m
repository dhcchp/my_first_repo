...
% Version 1.1, Peng CHEN, 2022.02.10; Rechecked to align with the verilog env.

%% Initial Parameter Settings
clc;clear;close all;
Sim_Time = 2^17; % The number is in FREF cycles.
...
I_path = 641;
%I_path2 = 1*3e8;
yiir1 = 0; yiir2 = 0; yiir3 = 0; yiir4 = 0;lambda1 = 1/2^2;lambda2 = 1/2^3;lambda3=1/2^0;lambda4=1/2^0;
accum_QE = 0;
tTDC_residue = 0;
...
for step = 1:Sim_Time
    ...
    trise(step) = -1*(tR(step) - tckv(step_DCO) - 1*tTDC_residue-1*2e-9);
    if (trise(step)>300e-8) trise(step) = 300e-8; elseif (trise(step)<-300e-8) trise(step) = -300e-8; end
    Tv_est = Tv;
    epsilon(step) = (floor(trise(step)/delta_TDC)-0.5*1);
    tTDC_residue = trise(step) - floor(trise(step)/delta_TDC)*delta_TDC;
    accum_QE = accum_QE + mash_out(step) - 1*FCW_F;
    if (abs(accum_QE) > 1e4) accum_QE = 1e4; end
    
    NC(step) = accum_QE;
    accum_QE_prev2 = accum_QE_prev1;
    accum_QE_prev1 = accum_QE;
    
    phie(step) =  epsilon(step)-1*Tv_est/delta_TDC*NC(step); %pay attention to NC is already normalized while epsilon is not.
...
%% OTW normalization    
   ...
   % if (otw(step+1)<150) otw(step+1) = 150;end % 8GHz/52MHz
   % if (otw(step+1)>480) otw(step+1) = 480;end %12GHz/25MHz
