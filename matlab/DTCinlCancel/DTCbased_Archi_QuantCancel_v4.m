...
% This version tries to solve small FCW_F issue by toggling FREF/CKVD

% Version 4, Peng CHEN, 2021.12.20
...
f0 = 2.4e9;
T0 = 1/f0;
fcw = 50+1/2^8;%When FCW_F is too small, the INL calibration does not work. It is related to the PLL LF coefficients.
...
phie = zeros(size(Rr));
mash_out = order2_mash(0.5,Sim_Time);
DCW = zeros(size(Rr));
...
DTC_gain_est = 0.99*DTC_gain;
rng(23);
DTC_DNL = 0*4e-13*randn(1,floor(1.7*floor(Tv/DTC_reso)));
...
for step = 1:Sim_Time
    ...
    PHRF = mod(PHRF,1);
    if (mod(step,2)==0)
        DCW(step) = floor((PHRF)*DTC_gain_est)+1;
        DCW_residue = (PHRF)*DTC_gain_est +1 - DCW(step);        
    else
    ...
    if step == 1  
        ...
        tR(step) = step * Tref + delta_Tref(step)+(2*(mod(step,2))-1)*DTC_delay(step);        
        tR_norm(step) = step * Tref;        
    end
    CNT_cnt = 0;
    ...
    trise(step) = (-tR(step) + tckv(step_DCO)+1*tckv_adjust*tckv_period(step_DCO) - 200e-12);
    Tv_est = 1.0*Tv; 
...
%     end    
    ...
    if (mod(step,2))
        epsilon(step) = (floor(trise(step)/delta_TDC)-0.5)*delta_TDC/Tv_est - (CNT_cnt + (2*(mod(step,2)==0)-1) - FCW_I - OV_PHRF) -1*DCW_residue/DTC_gain_est +0*INL_correction(DCW(step))/Tv_est;
    else
        epsilon(step) = (floor(trise(step)/delta_TDC)-0.5)*delta_TDC/Tv_est - (CNT_cnt + (2*(mod(step,2)==0)-1) - FCW_I - OV_PHRF) +1*(DCW_residue-2)/DTC_gain_est; %?????
    end
    phie(step) =  epsilon(step);
    if ((step_DCO>settle_sample*0.1) && mod(step,2)) DTC_gain_est = DTC_gain_est + 2*3e-4*sign(phie(step))*sign(1-PHRF-0.5); end
    if (DTC_gain_est <= 0.5*DTC_gain) DTC_gain_est = 0.5*DTC_gain; elseif (DTC_gain_est >= 1.5*DTC_gain) DTC_gain_est = 1.5*DTC_gain; end 
    
    if ((step_DCO>settle_sample*0.7))
    INL_correction(DCW(step)) = INL_correction(DCW(step)) - 2*1e-12*phie(step);end
