...
phie = zeros(size(Rr));
[DPDS_D,DPDS_R,DF_MMD] = PDS_SDM(FCW_F,Sim_Time);
PHF_R1 = zeros(size(Rr));
PHF_D1 = zeros(size(Rr));
PHF_R2 = zeros(size(Rr));
PHF_D2 = zeros(size(Rr));
PHF_R3 = zeros(size(Rr));
PHF_D3 = zeros(size(Rr));
...
gain_DTCR3_est = 0;
gain_DTCD3_est = 0;
DTCR_DNL = 1e-13*randn(1,floor(6.7*floor(Tv/DTCR_reso)));
DTCR_INL = cumsum(DTCR_DNL);
DTCR_INL = 2e-12*sin(3*pi*[1:length(DTCR_DNL)]/length(DTCR_DNL));
DTCD_DNL = 1e-13*randn(1,floor(6.7*floor(Tv/DTCD_reso)));
DTCD_INL = cumsum(DTCD_DNL);
DTCD_INL = 2e-12*sin(2*pi*[1:length(DTCD_DNL)]/length(DTCD_DNL));
tmp = zeros(size(Rr));
tmp2 = zeros(size(Rr));
tmp3 = zeros(size(Rr));
local_cnt = 0;
...
for step = 1:Sim_Time    
    if step == 1  
        ...
        Delta_TDEV_fast(step) = T0 - 1/(f0 + 16*phie(step-1)*fref/KDCO*KDCO);
        Delta_TDEV(step) = T0 - 1/(f0 + 1*otw(step)*KDCO);    
        tR(step) = step * Tref + delta_Tref(step);        
        tR_norm(step) = step * Tref;        
    end  
    div_cnt = 0;
    while div_cnt < (FCW_I + DF_MMD(step))   
        step_DCO = step_DCO + 1;
        if step_DCO == 1           
    ...
    DCWR1(step) = DPDS_R(step)*gain_DTCR_est;
    DCWD1(step) = DPDS_D(step)*gain_DTCD_est;
    
    PHF_R2(step) = DPDS_R(step)^2;
    PHF_D2(step) = DPDS_D(step)^2;
    DCWR2(step) = PHF_R2(step)*gain_DTCR2_est;
    DCWD2(step) = PHF_D2(step)*gain_DTCD2_est;
    PHF_R3(step) = DPDS_R(step)^3;
    PHF_D3(step) = DPDS_D(step)^3;
    DCWR3(step) = PHF_R3(step)*gain_DTCR3_est;
    DCWD3(step) = PHF_D3(step)*gain_DTCD3_est;    
    
    DCWR(step) = DCWR1(step) + 1*DCWR2(step) + 1*DCWR3(step);
    DCWD(step) = DCWD1(step) + 1*DCWD2(step) + 1*DCWD3(step);
    
    t_DTCr(step) = floor(DCWR(step))*DTCR_reso + DTCR_INL(1+floor(length(DTCR_INL)/2+DCWR(step)));
    t_DTCd(step) = floor(DCWD(step))*DTCD_reso + DTCD_INL(1+floor(length(DTCR_INL)/2+DCWD(step)));
	tmp(step) = gain_DTCR_est;tmp2(step) = gain_DTCD2_est;tmp3(step) = gain_DTCD3_est;
   ...
   % trise(step) = trise(step) + (trise(step)/2e-10)^2;
    ...
    gain_DTCR_est = gain_DTCR_est + (1)*1e-0*DPDS_R(step)*phie(step);
    gain_DTCD_est = gain_DTCD_est + (-1)*1e-0*DPDS_D(step)*phie(step);
    gain_DTCR2_est = gain_DTCR2_est + (1)*1e-0*PHF_R2(step)*phie(step);
    gain_DTCD2_est = gain_DTCD2_est + (-1)*1e-0*PHF_D2(step)*phie(step);
    gain_DTCR3_est = gain_DTCR3_est + (1)*1e-0*PHF_R3(step)*phie(step);
    gain_DTCD3_est = gain_DTCD3_est + (-1)*1e-0*PHF_D3(step)*phie(step);    
