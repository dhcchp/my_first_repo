...
% cancellation.
% This is the fundamental version. Only with DTC quantization noise ideally
% cancelled.
...
phie = zeros(size(Rr));
mash_out = order3_mash(FCW_F,Sim_Time);
DCW = zeros(size(Rr));
...
for step = 1:Sim_Time
    ...
    OV_PHRF = floor(PHRF);
    PHRF = mod(PHRF,1);
    DCW(step) = floor((1-PHRF)*DTC_gain);
    DTC_delay(step) = DCW(step)*DTC_reso;
    DCW_residue = (1-PHRF)*DTC_gain - DCW(step);
    if step == 1  
    ...
    trise(step) = (-tR(step) + tckv(step_DCO)-200e-12) - (CNT_cnt - FCW_I - OV_PHRF)*Tv - DCW_residue*DTC_reso;
    Tv_est = Tv;
    ...
    tmp(step) = CNT_cnt - 1*FCW_I - 1*OV_PHRF;
